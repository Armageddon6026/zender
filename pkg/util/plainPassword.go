...
//
// Code source : https://www.inanzzz.com/index.php/post/fxzc/generating-a-strong-random-plain-password-with-golang
func GeneratePlainPassword(length int) (string, error) {
	...
	for i := 1; i <= length; i++ {
		switch group {
		case 0:
			pick(0, 30, retry, &chars, rng)
			group++
		case 1:
			pick(1, 10, retry, &chars, rng)
			group++
		case 2:
			pick(2, 26, retry, &chars, rng)
			group++
		case 3:
			pick(3, 26, retry, &chars, rng)
			group = 0
		default:
...
func pick(key, len, retry int, chars *string, rng *rand.Rand) {
	var try int
